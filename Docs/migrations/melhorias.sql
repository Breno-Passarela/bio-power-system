...
CREATE TABLE IF NOT EXISTS tb_status_diversos (
...
INSERT IGNORE INTO tb_status_diversos (sta_dominio, sta_codigo, sta_descricao) VALUES
  -- Agendamentos
...
SET @sta_age_default := (SELECT sta_id FROM tb_status_diversos WHERE sta_dominio='agendamento_status' AND sta_codigo='AGUARDANDO_PAGAMENTO');
SET @sta_dev_default := (SELECT sta_id FROM tb_status_diversos WHERE sta_dominio='devolucao_status' AND sta_codigo='PENDENTE');
SET @sta_ped_default := (SELECT sta_id FROM tb_status_diversos WHERE sta_dominio='pedido_status' AND sta_codigo='SOLICITADO');
SET @sta_ven_status_default := (SELECT sta_id FROM tb_status_diversos WHERE sta_dominio='venda_status' AND sta_codigo='AGUARDANDO');
SET @sta_ven_metodo_default := (SELECT sta_id FROM tb_status_diversos WHERE sta_dominio='venda_metodo_pagamento' AND sta_codigo='PIX');
...
UPDATE tb_Agendamentos a
  JOIN tb_status_diversos s ON s.sta_dominio='agendamento_status' AND s.sta_codigo=a.age_status
...
ALTER TABLE tb_Agendamentos
  ...
  ADD CONSTRAINT fk_age_status FOREIGN KEY (age_status_id) REFERENCES tb_status_diversos(sta_id);
...
UPDATE tb_Devolucoes d
  JOIN tb_status_diversos s ON s.sta_dominio='devolucao_status' AND s.sta_codigo=d.dev_status
...
ALTER TABLE tb_Devolucoes
  ...
  ADD CONSTRAINT fk_dev_status FOREIGN KEY (dev_status_id) REFERENCES tb_status_diversos(sta_id);
...
UPDATE tb_Fluxo_Caixa f
  JOIN tb_status_diversos s ON s.sta_dominio='fluxo_caixa_tipo' AND s.sta_codigo=f.flu_tipo
...
ALTER TABLE tb_Fluxo_Caixa
  ...
  ADD CONSTRAINT fk_flu_tipo FOREIGN KEY (flu_tipo_id) REFERENCES tb_status_diversos(sta_id);
...
UPDATE tb_Pedidos_Compra p
  JOIN tb_status_diversos s ON s.sta_dominio='pedido_status' AND s.sta_codigo=p.ped_status
...
ALTER TABLE tb_Pedidos_Compra
  ...
  ADD CONSTRAINT fk_ped_status FOREIGN KEY (ped_status_id) REFERENCES tb_status_diversos(sta_id);
...
UPDATE tb_Vendas v
  LEFT JOIN tb_status_diversos sm ON sm.sta_dominio='venda_metodo_pagamento' AND sm.sta_codigo=v.ven_metodo_pagamento
  LEFT JOIN tb_status_diversos ss ON ss.sta_dominio='venda_status' AND ss.sta_codigo=v.ven_status_venda
...
ALTER TABLE tb_Vendas
  ...
  ADD CONSTRAINT fk_ven_metodo_pagamento FOREIGN KEY (ven_metodo_pagamento_id) REFERENCES tb_status_diversos(sta_id),
  ADD CONSTRAINT fk_ven_status FOREIGN KEY (ven_status_id) REFERENCES tb_status_diversos(sta_id);
...
-- 11) Flag de ativacao de usuarios.
ALTER TABLE tb_Usuarios
  ADD COLUMN IF NOT EXISTS usu_ativo TINYINT(1) NOT NULL DEFAULT 1 AFTER usu_typ_id;

COMMIT;

-- Consultas utilitarias pos-migracao
-- SELECT * FROM tb_status_diversos ORDER BY sta_dominio, sta_codigo;
-- SELECT sta_codigo FROM tb_status_diversos WHERE sta_dominio='venda_status';
